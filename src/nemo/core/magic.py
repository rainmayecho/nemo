...
def regenerate_magic():
    ...
    nl = "\n"
    tab = "\t"
    print(f"BISHOP_MAGIC = [{nl}{tab}{f',{nl}{tab}'.join(f'Bitboard({hex(bb)})' for bb in  BISHOP_MAGIC)}{nl}]")
    print(f"ROOK_MAGIC = [{nl}{tab}{f',{nl}{tab}'.join(f'Bitboard({hex(bb)})' for bb in ROOK_MAGIC)}{nl}]")
